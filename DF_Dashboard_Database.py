...
con = sqlite3.connect("Test Database.db")  # Name of database
df = pd.read_sql_query("SELECT * FROM Main_Sensors", con)
...
df = df.dropna()
# changing the Latitude column to float
df.latitude = df.latitude.astype(float)
df.longitude = df.longitude.astype(float)

print(df)

df['rolling_mean_pm10'] = df['standard_pm10'].rolling(5).mean()

# drops NAN records
df = df.replace(' nan', np.nan)
df = df.dropna()

# get coordinates for center of map
latitude = df['latitude'].values[1]
longitude = df['longitude'].values[1]
...
app.layout = dbc.Container([
    ...
    dbc.Row(html.Img(src="assets/DFH_Logo.png", height="85px")),  # branding logo
...
def display_graph(n_clicks):  # Renders the graph data-points
    ...
    x_pm_10, y_pm_10 = 'gps_date', 'rolling_mean_pm10'
    ...
    fig_pm_10 = px.line(df, x=x_pm_10, y=y_pm_10, range_x=[])
    fig_pm_10.update_xaxes(showticklabels=False, title=None)
    ...
    fig_pm_10.update_yaxes(range=[0, 40])
    ...
    fig_temperature.add_hrect(y0=32, y1=40, line_width=0, fillcolor="blue", opacity=0.2)
    fig_temperature.add_hrect(y0=40, y1=45, line_width=0, fillcolor="green", opacity=0.2)
    fig_temperature.add_hrect(y0=45, y1=55, line_width=0, fillcolor="darkgreen", opacity=0.2)
    ...
    fig_mapbox = px.density_mapbox(df, lat="latitude", lon="longitude", z="standard_pm2_5",
                                   hover_data=["alt", "standard_pm1_0"], zoom=11, height=500)
